...
def deploy():
    ...
    deploy.add('wireguard1', charm='wireguard')
    deploy.add('wireguard2', charm='wireguard')
    # deploy.expose('wireguard')
    deploy.configure('wireguard2', {
...
@pytest.fixture(scope="module")
def units(deploy):
    return deploy.sentry['wireguard']
